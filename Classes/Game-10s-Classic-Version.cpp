...
void Game::GenerateBonus()
{
	...
	//Pause Timers. Remove from timers vector so resuming app does not unpause them
	countdownTimer.Stop();
	sceneTimer.Stop();
	sequenceTimer.Stop();
	matchTimer.Stop();
...
void Game::DisplayBonus()
{
	...
	//Allow designnated bonus if testing
...
bool Game::CheckForAchievements()
{
    ...
    if(testing && testingAchievements)
    {
		...
		achievementsUnlocked++;
		achievementsText.push_back(nrgText::create("Golf Rules\n\nBeat Practice Mode with a score of 500 points. If patience is a virtue, you have a lot of virtue!", "fonts/POE.ttf", achievementTextSize, baseTile->GetWidth() - 10, 0, 0, cocos2d::Color4B::WHITE, cocos2d::TextHAlignment::CENTER));
...
{
	if (gameData->getBoolForKey(app_rated))
		requestReview = true;
	else
		requestReview = false;
}
...
void Game::onEnter()
{
	//Make sure to resize window for Android
	...
	cocos2d::JniHelper::getStaticMethodInfo(javaMethodHelper, "org/cocos2dx/cpp/AppActivity", "hideSystemUI", "()V");
	javaMethodHelper.env->CallStaticVoidMethod(javaMethodHelper.classID, javaMethodHelper.methodID);
#endif
