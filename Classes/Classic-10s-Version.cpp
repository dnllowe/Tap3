...
void Classic::UpdateTimeLimit()
{
	if (!testing)
	{
		round = matches;

		//Get the first whole digit
		while (round >= 20 && round < 100)
			round -= 20;
...
void Classic::RandomizeTiles()
{
	...
	if (matches >= 80 && matchCriteria == COLORS)
	{
...
void Classic::SetMatchParameters(int excludedMatchCriteria, bool blind)
{
	...
	//Don't allow split color matches to have different style until match 60
	if (difficulty >= 40 && difficulty < 60 && matchCriteria == COLORS)
	{
		useDesignatedMatchStyle = true;
		useDesignatedNonmatchStyle = true;
	...
	//After Match 80, color matches have tricky set
